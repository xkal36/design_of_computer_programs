...
# Fixing play_pig so that an incorrect input
# is handled correctly, as opposed to before,
# where we played still played the game as 
# if we had a proper input. This highlights
# one of the disadvantages of dynamic typing.
# In a statically typed language, the program
# simply wouldn't have compiled.
...
# Better way:
def play_pig_better(A, B, dierolls=dierolls()):
    """Play a game of pig between two players, represented by their strategies.
    Each time through the main loop we ask the current player for one decision,
    which must be 'hold' or 'roll', and we update the state accordingly.
    When one player's score exceeds the goal, return that player."""
    ...
    while True:
        ...
        if me >= goal:
            ...
            if action == 'hold':
                ...
                state = roll(state, next(dierolls))
            else: # illegal strategy/move entered: immediately lose
                return strategies[other[p]]
