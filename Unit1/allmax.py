...
def allmax(iterable, key=None):
    ...
    mapped = map(key, iterable)
    maxes = []
    for i in range(len(iterable)):
        if mapped[i] == max(mapped):
            maxes.append(iterable[i])
    return maxes

# My solution (using list comprehension instead):
def allmax_better(iterable, key=None):
    "Return a list of all items equal to the max of the iterable."
    mapped = map(key, iterable)
    return [iterable[i] for i in range(len(iterable)) if mapped[i] == max(mapped)]

# Instructor's solution:
def allmax_ins(iterable, key=None):
    "Return a list of all items equal to the max of the iterable."
    result, maxval = [], None
...
# Best solution:
def allmax_best(iterable, key=lambda x:x):
    "Return a list of all items equal to the max of the iterable."
