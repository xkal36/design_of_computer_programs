...
def mc_problem2(start=(3, 3, 1, 0, 0, 0), goal=None):
    ...
    if goal is None:
        goal = (0, 0, 0) + start[:3]
    mc_goal = lambda state: state == goal
    return shortest_path_search(start, csuccessors, mc_goal)

# Better answer:
def mc_problem2_better(start=(3, 3, 1, 0, 0, 0), goal=None):
    """
    Solve the missionaries and cannibals problem.
    State is 6 ints: (M1, C1, B1, M2, C2, B2) on the start (1) and other (2) sides.
    Find a path that goes from the initial state to the goal state (which, if
...
# Peter's answer:
def mc_problem2_peter(start = (3, 3, 1, 0, 0, 0), goal = None):
    """
    Solve the missionaries and cannibals problem.
    State is 6 ints: (M1, C1, B1, M2, C2, B2) on the start (1) and other (2) sides.
    Find a path that goes from the initial state to the goal state (which, if
